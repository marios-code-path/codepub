...
~~~~~~~~~~~~~~~~~~~~~
Riff works by routing arguments via pub/sub. A function controller listens
to requests via http and routes parameters to  listen to specific
topics for input and 
to a
Riff basically works by routing arguments across pipes (backed
by your favorite Kafka). These pipes respond to a pub/sub ] functions writtin in [ Java et al., Python, Javascript,
shell ]. This demonstration will use 
get to the rest later :). To demonstrate a [fn] workflow I will implement
3 common funcional types - Function, Supplier, Consumer. 
...
Executing The riff function
----
In order to get all this working, we wiull need to create
the Riff service images and have them deployed.
use the   `riff` command to do this automatically. 
Example to setup our DemoSupplier function:

.RiffSetup-Supplier.sh
[source,script]
----
include::demofn/inst.sh
----